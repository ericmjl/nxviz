...
headers = ['some_num', 'some_string', 'ten_things', 'color', 'group', 'location']
data_generator = ((
    i,
    str(i),
    i % 10,
    ...
    choice(['A', 'B']),
    choice(['New York', 'San Francisco', 'Chicago', 'LA', 'Austin', 'Vancouver'])
...
def test_node_properties():
    g = graph_from_dataframe(test_dataframe, threshold_by_count_unique=3, node_property_columns=['ten_things'])
...
def test_edge_properties():
    g = graph_from_dataframe(test_dataframe, edge_property_columns=['ten_things', 'some_string'])

    assert g.number_of_nodes() == 21
    assert g.number_of_edges() == 129
    assert g['blue']['B'] == {'some_string': '96', 'ten_things': 6, 'type': 'color_group', 'weight': 20.0}
